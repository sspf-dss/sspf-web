...
import { DrawerService, HasDrawerTemplates } from '@sspf/ngx-drawer';
...
export class AddressDrawerComponent implements HasDrawerTemplates, OnInit {
  ...
  onProvinceChange(provinceId: string | undefined) {
    if (provinceId) {
  ...
  onDistrictChange(districtId: string | undefined) {
    if (districtId) {
  ...
  onSubdistrictChange(subdistrictId: string | undefined) {
    if (subdistrictId) {
